...
function CarouselNavigation({ className, classNameButton, alwaysShow }: CarouselNavigationProps) {
  ...
  return (
    <div className={cn('pointer-events-none absolute top-1/2 left-0 flex w-full -translate-y-1/2 justify-between px-2', className)}>
      <button
        type="button"
...
function CarouselContent({ children, className, transition }: CarouselContentProps) {
  ...
  const itemsLength = Children.toArray(children).filter(child => child != null).length;
